...
// Variables globales para expansión
...
/**
 * SOLUCIÓN DEFINITIVA - Animación JavaScript Pura
 */
async function processSeed() {
    ...
    console.log('🎬 DEFINITIVO: JavaScript animation...');

    let startTime = Date.now();
    let isAnimating = false;
    let animationPosition = -40; // Posición inicial
    ...
    try {
        ...
        if (progressBar && progressFill) {
            console.log('📊 Setting up JS-controlled animation...');
            ...
            // FUNCIÓN DE ANIMACIÓN JAVASCRIPT
            const animateBar = () => {
        ...
        // Delay para establecer animación
        await new Promise(resolve => setTimeout(resolve, 300));

        startTime = Date.now();

        console.log('🔐 Calling ASYNC Argon2id with JS animation...');
        ...
        console.log(`✅ Async Argon2id completed in: ${processingTime}ms`);

        // PARAR ANIMACIÓN Y COMPLETAR
        isAnimating = false;

        if (progressFill) {
            console.log('📊 === FIXED: Starting completion transition ===');

            // RESET COMPLETO
            progressFill.style.cssText = '';
            progressBar.offsetHeight; // Force reflow

            // Aplicar estilos SIN transición primero
            progressFill.style.position = 'static';
            progressFill.style.width = '0%';
            progressFill.style.height = '100%';
            progressFill.style.background = 'linear-gradient(90deg, #ff9500, #ff7b00)';
            progressFill.style.borderRadius = '3px';
            progressFill.style.transition = 'none'; // SIN transición inicialmente

            // Force reflow para establecer 0%
            progressFill.offsetWidth;

            console.log('📊 Width set to 0% without transition:', getComputedStyle(progressFill).width);

            // AHORA agregar transición y animar
            setTimeout(() => {
                if (progressFill) {
                    console.log('📊 Adding transition and animating to 100%...');
                    progressFill.style.transition = 'width 1s ease-out';

                    // Force reflow después de agregar transición
                    progressFill.offsetWidth;
                    ...
                    setTimeout(() => {
                        if (progressFill) {
                            ...
                            console.log('📊 Width changed to 100%');

                            // Check final
                            setTimeout(() => {
                                console.log('📊 FINAL width:', getComputedStyle(progressFill).width);
                            }, 1200);
                        }
                    }, 50);
                }
        ...
        if (result.success && result.result) {
            ...
            if (statusText) {
                statusText.textContent = `Ready • Processing: ${processingTime}ms • ${currentWords.length} words`;
            }
...
async function deriveAddresses() {
    ...
    const addressCount = parseInt(addressCountElement.value) || 1; // Default a 1 si es inválido
    ...
    try {
        console.log('🚀 Calling derive_addresses with address count:', {
        ...
        console.log('✅ Derivation result:', result);

        if (AppState.processing.active) {
            resetExpansionState(); // Resetear estado de expansión
            AppState.derivationResults = result;
            displayResults(result, selectedSource);
            initializeBitcoinTabs(); // Inicializar pestañas Bitcoin
...
async function generateDetailedView(results) {
    ...
    for (const [network, addresses] of Object.entries(results)) {
        ...
        if (network === 'bitcoin') {
            ...
            // Crear contenido para cada tipo CON DISPLAY BLOCK/NONE EXPLÍCITO
            Object.entries(groupedBitcoin).forEach(([type, typeAddresses]) => {
                ...
                typeAddresses.forEach((addr, index) => {
                    // Extraer número del address_type
                    ...
                    html += `
                        <div class="address-item">
                            ...
                            <div class="address-actions">
                                <button class="action-button" onclick="copyToClipboard('${addr.address}')">📋</button>
                                <button class="action-button" onclick="showAddressInfo('${addr.address_type}', '${addr.path}', '${addr.address}')">ℹ️</button>
                            </div>
                        </div>
                    `;
                });
            ...
            // TRATAMIENTO NORMAL PARA OTRAS REDES
            html += `
...
/**
 * Bitcoin Tab Functions
 */
function switchBitcoinTab(tabType) {
    console.log('🔄 Switching Bitcoin tab to:', tabType);
    ...
    // Actualizar contenido de pestañas - MOSTRAR/OCULTAR INMEDIATAMENTE
    document.querySelectorAll('.bitcoin-tab-content').forEach(content => {
        content.style.display = 'none'; // Forzar ocultación
        content.classList.remove('active');
    });

    const activeContent = document.getElementById(`bitcoin-tab-${tabType}`);
    if (activeContent) {
        activeContent.style.display = 'block'; // Forzar visualización
        activeContent.classList.add('active');
    }

    console.log(`✅ Bitcoin tab switched to ${tabType}`);
}

function switchCompactBitcoinTab(network, tabType) {
    console.log('🔄 Switching compact Bitcoin tab:', network, tabType);

    // Actualizar botones de pestañas compactas
    ...
    // Actualizar contenido de pestañas compactas - MOSTRAR/OCULTAR INMEDIATAMENTE
    ...
    console.log(`✅ Compact Bitcoin tab switched to ${tabType} for ${network}`);
}
...
async function generateCompactViewWithIcons(results) {
    ...
    for (const [network, addresses] of Object.entries(results)) {
        ...
        // Si está expandida, mostrar las direcciones
        if (isExpanded) {
            ...
            if (network === 'bitcoin') {
                ...
                // Mini pestañas para compact view
                html += `
                    ...
                    typeAddresses.forEach((addr, index) => {
                        ...
                        html += `
                            <div class="compact-address-item" style="margin-bottom: 6px; display: flex; align-items: center; font-size: 0.85em;">
                                ...
                                <button class="action-button" onclick="copyAddress('${addr.address}')" style="background: #333; border: 1px solid #555; color: white; padding: 2px 6px; border-radius: 3px; cursor: pointer; font-size: 0.8em;">📋</button>
                            </div>
                        `;
                    });
                ...
                addresses.forEach((addr, index) => {
                    ...
                    html += `
                        <div class="address-item" style="margin-bottom: 8px; display: flex; align-items: center;">
                            ...
                            <button class="action-button" onclick="copyAddress('${addr.address}')" style="background: #333; border: 1px solid #555; color: white; padding: 4px 8px; border-radius: 4px; cursor: pointer;">📋</button>
                        </div>
                    `;
                });
            }
...
function toggleNetworkExpansion(network) {
    ...
    if (isExpanded) {
        ...
        // Para Bitcoin, asegurar que la pestaña correcta esté activa
        if (network === 'bitcoin') {
...
function expandNetwork(network) {
    ...
    if (viewMode && viewMode.value === 'compact') {
        // Si estamos en compact view, togglear expansión
...
/**
 * Precargar iconos críticos para mejor rendimiento
 */
async function preloadCriticalIcons() {
    ...
    console.log('✅ Critical icons preloaded');
}

/**
 * Función de utilidad para debugging de iconos
 */
function debugIconLoading() {
    console.log('🔍 Icon cache status:');
    ...
    networks.forEach(async (network) => {
        ...
        try {
            const svgContent = await loadSVGIcon(networkInfo.icon);
            console.log(`✅ ${network}: ${svgContent ? 'Loaded' : 'Failed'}`);
        } catch (error) {
...
async function copyAllNetworkAddresses(network) {
    ...
    if (network === 'bitcoin') {
        // Para Bitcoin, agrupar por tipo si hay múltiples tipos
        const grouped = {
            'Legacy P2PKH': addresses.filter(addr => addr.address_type.includes('Legacy P2PKH')),
            'Nested SegWit': addresses.filter(addr => addr.address_type.includes('Nested SegWit')),
            'Native SegWit': addresses.filter(addr => addr.address_type.includes('Native SegWit'))
        ...
        // Si estamos en una pestaña específica, copiar solo esa
...
async function generateExpandableResults(results) {
    ...
    for (const [network, addresses] of Object.entries(results)) {
            ...
            <div class="network-result-container" data-network="${network}">
                <div class="network-result-header ${isExpanded ? 'expanded' : ''}"
                    ...
                    <div class="network-info">
                        <span class="expansion-indicator">${isExpanded ? '▼' : '▶'}</span>
                        ${iconHtml}
                        <span class="network-name">${networkInfo.name}</span>
        ...
        // Contenido específico según el tipo de red
        if (network === 'bitcoin') {
...
async function generateBitcoinExpandedContent(network, addresses) {
    ...
    const activeTab = activeBitcoinTabs.get(network) || 'legacy'; // CAMBIAR A 'legacy'

    let html = `
        <div class="bitcoin-tabs-in-expanded">
            <div class="bitcoin-tabs-header">
                <button class="bitcoin-tab-btn ${activeTab === 'legacy' ? 'active' : ''}"
                        onclick="switchBitcoinTab('legacy')" data-tab="legacy">
                    Legacy P2PKH (${grouped.legacy.length})
                </button>
                <button class="bitcoin-tab-btn ${activeTab === 'nested' ? 'active' : ''}"
                        onclick="switchBitcoinTab('nested')" data-tab="nested">
                    Nested SegWit (${grouped.nested.length})
                </button>
                <button class="bitcoin-tab-btn ${activeTab === 'native' ? 'active' : ''}"
                        onclick="switchBitcoinTab('native')" data-tab="native">
                    Native SegWit (${grouped.native.length})
                </button>
            </div>
    ...
    // Generar contenido para cada pestaña con IDs y clases correctas
    Object.entries(grouped).forEach(([type, typeAddresses]) => {
        ...
        typeAddresses.forEach((addr, index) => {
            ...
            html += `
                <div class="address-item">
                    <div class="address-info">
                        <span class="address-index">#${addressNumber}.</span>
                        <div class="address-details">
                            <div class="address-label">
                                ${addr.address}
                                ${isMain ? ' <span class="master-badge">Master</span>' : ''}
                            </div>
                            <div class="address-path">Path: ${addr.path}</div>
                        </div>
                    </div>
    ...
    addresses.forEach((addr, index) => {
        ...
        html += `
            <div class="address-row ${isMain ? 'main-address' : ''}">
                ...
                <span class="address-value" title="${addr.address}">
                    ${addr.address}
                </span>
...
function copyBitcoinByActiveTab(network) {
    ...
    // Filtrar direcciones según pestaña activa
    let targetAddresses = [];
    switch (activeTab) {
        case 'legacy':
...
function initializeBitcoinTabs() {
    ...
    bitcoinNetworks.forEach(container => {
        ...
        // Si ya está expandida, aplicar el estado
...
function handleNetworkHeaderClick(event, network) {
    // Solo expandir/colapsar si no se clickeó un botón de acción
...
function scrollToNetworkIfNeeded(network) {
    ...
    // Si el contenedor no está completamente visible, hacer scroll
...
/**
 * =============================================================================
 * EXPORTACIONES GLOBALES - CONSOLIDACIÓN FINAL
 * =============================================================================
 * Todas las funciones que necesitan ser accesibles desde onclick handlers
 * y otros contextos globales se exportan aquí al final del archivo
 */
...
// ===== FUNCIONES DE SELECCIÓN DE REDES =====
...
// ===== FUNCIONES DE PESTAÑAS BITCOIN (CRÍTICAS) =====
...
// ===== FUNCIONES DE RESULTADOS Y NAVEGACIÓN =====
...
// ===== FUNCIONES DE DEBUGGING (DESARROLLO) =====
window.debugIconLoading = debugIconLoading;

/**
 * Application Ready - Mensaje final
 */
console.log('📱 SCypher GUI v3.0 - Modular architecture loaded');
console.log('🔧 Components: Templates, Utils, Components, App');
console.log('🎯 Ready for user interaction');
console.log('✅ Global functions exported:', Object.keys(window).filter(key =>
    typeof window[key] === 'function' &&
    ['processSeed', 'deriveAddresses', 'switchBitcoinTab', 'expandNetwork'].includes(key)
).length, 'critical functions');
